...
#This script must be run under the directory it exists
#Run this script after you download the php source code from php.net
#and extract the source code to dist directory
...
ls -1 |grep -E ".*-([[:digit:]]+\.+){2}"|grep -v tgz| while read i ; do mv $i `echo $i | cut -d\- -f 1` ; done
